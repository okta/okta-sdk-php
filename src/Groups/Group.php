...
class Group extends \Okta\Resource\AbstractResource
{
    ...
    */
    public function getApplications(array $options = []): \Okta\Groups\Collection
    {

        return $this
                ->getDataStore()
                ->getCollection(
                    "/api/v1/groups/{$this->getId()}/apps",
                    \Okta\Groups\Application::class,
    ...
    */
    ...
    {
        $uri = "/api/v1/groups/{$this->getId()}/roles";
        $uri = $this->getDataStore()->buildUri(
            $this->getDataStore()->getOrganizationUrl() . $uri
        );
        $body = $this
        ...
        return new \Okta\Groups\Role(null, $body);
    }
}
